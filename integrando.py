...
#inserindo em grupos (batch) de 10k
batch_size = 10000
for i in range(1, 1000001):
    status_texto = 'Sim' if i % 2 == 0 else 'Não'
    status_booleano = 1 if i % 2 == 0 else 0
    cursor.execute('INSERT INTO teste (status_texto, status_booleano) VALUES (?, ?)',
...
# Testar consulta com texto
start = time.time()
cursor.execute("SELECT COUNT(*) FROM teste WHERE status_texto = 'Sim'")
...
# Testar consulta com booleano
start = time.time()
cursor.execute("SELECT COUNT(*) FROM teste WHERE status_booleano = 1")
